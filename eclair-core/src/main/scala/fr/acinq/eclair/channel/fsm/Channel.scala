          ...
          if (d.commitments.changes.remoteHasUnsignedOutgoingHtlcs) {
            ...
            if (d.commitments.hasNoPendingHtlcsOrFeeUpdate) {
              ...
              if (Features.canUseFeature(d.commitments.params.localParams.initFeatures, d.commitments.params.remoteParams.initFeatures, Features.SimpleClose)) {
                ...
                val (closingTx, closingSigned) = MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, localShutdown.scriptPubKey, remoteShutdownScript, nodeParams.currentFeeratesForFundingClosing, nodeParams.onChainFeeConf, d.closingFeerates, localClosingNonce, remoteClosingNonce)
...
//                  this.pendingRemoteNextLocalNonce = msg.secondRemoteNonce
            ...
            case Right((commitments1, revocation)) =>
              ...
              if (commitments1.hasNoPendingHtlcsOrFeeUpdate) {
                if (Features.canUseFeature(d.commitments.params.localParams.initFeatures, d.commitments.params.remoteParams.initFeatures, Features.SimpleClose)) {
                  ...
                  // we pay the closing fees, so we initiate the negotiation by sending the first closing_signed
                  val (closingTx, closingSigned) = MutualClose.makeFirstClosingTx(keyManager, commitments1.latest, localShutdown.scriptPubKey, remoteShutdown.scriptPubKey, nodeParams.currentFeeratesForFundingClosing, nodeParams.onChainFeeConf, closingFeerates, localClosingNonce, remoteShutdown.shutdownNonce_opt)
  ...
  when(NEGOTIATING)(handleExceptions {
    ...
    case Event(c: ClosingSigned, d: DATA_NEGOTIATING) if d.commitments.params.commitmentFormat.useTaproot =>
      ...
      Closing.MutualClose.checkClosingSignature(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, c.feeSatoshis, localClosingNonce.get, d.remoteShutdown.shutdownNonce_opt.get, remotePartialSignature.partialSignature) match {
            ...
            c.feeRange_opt match {
              case Some(ClosingSignedTlv.FeeRange(minFee, maxFee)) if !d.commitments.params.localParams.paysClosingFees =>
                ...
                if (maxFee < localClosingFees.min) {
                  ...
                  if (closingFee == remoteClosingFee) {
                    ...
                    val (closingTx, closingSigned) = MutualClose.makeClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, ClosingFees(closingFee, minFee, maxFee), localClosingNonce, d.remoteShutdown.shutdownNonce_opt)
              ...
              case _ =>
                ...
                val (closingTx, closingSigned) = {
                  ...
                  Closing.MutualClose.makeClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, localClosingFees.copy(preferred = nextPreferredFee), localClosingNonce, d.remoteShutdown.shutdownNonce_opt)
                }
                val closingTxProposed1 = (d.closingTxProposed: @unchecked) match {
                  case previousNegotiations :+ currentNegotiation => previousNegotiations :+ (currentNegotiation :+ ClosingTxProposed(closingTx, closingSigned))
                }
      ...
      c.feerates match {
        case Some(feerates) =>
          if (c.scriptPubKey.nonEmpty && !c.scriptPubKey.contains(d.localShutdown.scriptPubKey)) {
            ...
            val (closingTx, closingSigned) = MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, nodeParams.currentFeeratesForFundingClosing, nodeParams.onChainFeeConf, Some(feerates), localClosingNonce, d.remoteShutdown.shutdownNonce_opt)
  ...
  when(OFFLINE)(handleExceptions {
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: ChannelDataWithCommitments) =>
      ...
      val nonceMap = mutable.HashMap.empty[TxId, IndividualNonce]
        ...
        val nonces1 = d match {
          case d: DATA_NORMAL => d.spliceStatus match {
            case w: SpliceStatus.SpliceWaitingForSigs =>
              ...
              val nonce = keyManager.verificationNonce(localFundingPubKey, w.signingSession.localCommitIndex + 1)._2
              nonce +: nonces
            case _ => nonces
          }
          case _ => nonces
  ...
  when(SYNCING)(handleExceptions {
    ...
    case Event(channelReestablish: ChannelReestablish, d: DATA_NEGOTIATING) =>
      ...
      if (d.commitments.params.localParams.paysClosingFees) {
        ...
        val (closingTx, closingSigned) = Closing.MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, nodeParams.currentFeeratesForFundingClosing, nodeParams.onChainFeeConf, None, localClosingNonce, d.remoteShutdown.shutdownNonce_opt)
