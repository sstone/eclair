    ...
    session.toSend match {
      ...
      case Nil =>
        ...
        val commitNonces = if (this.channelParams.commitmentFormat.useTaproot) {
          List(
            keyManager.verificationNonce(this.localFundingPubKey, this.purpose.localCommitIndex)._2,
            keyManager.verificationNonce(this.localFundingPubKey, this.purpose.localCommitIndex + 1)._2,
          )
        } else Nil
