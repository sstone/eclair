...
class Peer(nodeParams: NodeParams, remoteNodeId: PublicKey, authenticator: ActorRef, watcher: ActorRef, router: ActorRef, relayer: ActorRef, wallet: EclairWallet) extends FSMDiagnosticActorLogging[Peer.State, Peer.Data] {
  ...
  when(CONNECTED, stateTimeout = nodeParams.pingInterval) {
    ...
    case Event(msg: wire.RoutingMessage, ConnectedData(_, _, _, _, _, _, ignoreAnnouncements)) =>
      if (ignoreAnnouncements) {
        ...
        router ! PeerRoutingMessage(remoteNodeId, msg)
      }
      stay
    ...
    case Event(badMessage: BadMessage, data@ConnectedData(_, _, _, _, _, badMessages, _)) =>
      log.warning(s"peer sent us bad message: $badMessage")
      val ban = badMessage match {
        case FundingTxAlreadySpent(_) =>
          val count = badMessages.collect { case x: FundingTxAlreadySpent => x }.size + 1
          val max = 5
          if (count > max) {
            log.warning(s"peer sent us $count bad announcements, let's temporary ignore network announcements for 5 minutes")
            import scala.concurrent.ExecutionContext.Implicits.global
            context.system.scheduler.scheduleOnce(5 minutes, self, ResumeAnnouncements)
            true
          } else false
      }
      stay using data.copy(badMessages = badMessages :+ badMessage, ignoreAnnouncements = ban)

    case Event(ResumeAnnouncements, data: ConnectedData) =>
      log.info(s"resuming processing of network announcements for peer")
      stay using data.copy(badMessages = Seq.empty, ignoreAnnouncements = false)
...
object Peer {
  ...
  case class ConnectedData(address_opt: Option[InetSocketAddress], transport: ActorRef, remoteInit: wire.Init, channels: Map[ChannelId, ActorRef], gossipTimestampFilter: Option[GossipTimestampFilter] = None, badMessages: Seq[BadMessage] = Seq.empty, ignoreAnnouncements: Boolean = false) extends Data
